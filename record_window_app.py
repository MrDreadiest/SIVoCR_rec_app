...
class App(ctk.CTk):
    ...
    def initialize_variable(self):
        ...
        self.rec_param_frames_per_buffer :int = 512
        self.rec_param_frame_rate = 44100
        self.rec_param_channels = 1
        self.rec_param_format = pyaudio.paInt16
        ...
        self.command_list = ["Stój", "Ręce na głowę", "Odrzuć broń", ]
    ...
    def initialize_gui(self):
        ...
        self.gui_settings_side_bar_frame.grid(row=0, column=0, rowspan=2, sticky="ns")

        # create graph 1 frame with
        self.gui_wave_form_graph_frame = ctk.CTkFrame(self, corner_radius=0)
        self.gui_wave_form_graph_frame.grid(row=0, column=1, sticky="nswe",padx=10, pady=(0,5))
        ...
        self.gui_spectrogram_graph_frame.grid(row=1, column=1, sticky="nswe",padx=10, pady=(5,0))
    ...
    def get_file_name(self):
        return f"{self.speaker_sex_type_list[self.speaker_sex_type_index][0]}_{self.speaker_number:04d}_{self.command_index}_{self.command_attempt}.wav"
    ...
    def gui_update_labels(self):
        ...
        print(os.path.join(os.getcwd(),self.record_file_path))
    ...
    def input_save_button_callback(self):
        ...
        if  len(self.rec_data_0_frames) > 0:
            ...
            threading.Thread(target=self.play_record).start()
    ...
    def plot_wave(self, frames_len, frames :bytes):
        ...
        signal_array = np.frombuffer(wave_frames, dtype=np.int16)
